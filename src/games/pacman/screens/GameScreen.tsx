...
import { Direction, TileType, GhostStatus } from '../types';
import type { Position, GhostEntity } from '../types';
...
const GameScreen = ({ teamName, onGameOver, onGameWin }: GameScreenProps) => {
  ...
  const initRound = useCallback((roundNum: number, keepScore: boolean = true) => {
    ...
    const mapToUse = roundNum === 1 ? INITIAL_MAP : ROUND_2_MAP;
    const newGrid = mapToUse.map(row => [...row]);
    let pelletCount = 0;
  ...
  const update = (time: number) => {
    ...
    if (pY >= 0 && pY < MAP_HEIGHT && pX >= 0 && pX < MAP_WIDTH) {
       ...
       if (item === TileType.PELLET) {
         state.score += state.round === 2 ? 20 : 10;
    ...
    if ((state.round === 1 && state.score >= 1700) || (state.round === 2 && state.score >= 5000)) {
        handleLevelComplete();
        return; 
    }
    ...
    const ghostSpeedMult = state.round === 1 ? 0.85 : 0.95;
  ...
  const handleLevelComplete = () => {
     ...
     setTimeout(() => {
        if (gameState.current.round < 2) {
            initRound(gameState.current.round + 1, true);
        } else {
            // VICTORY CONDITION: End of Round 2
  ...
  // Determine current effective round ID (cycles 1-2)
  const currentThemeId = ((uiState.round - 1) % 2) + 1;
